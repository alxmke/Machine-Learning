...
class Perceptron(object):
    ...
    def predict(self, x):
        ...
        """
        "*** YOUR CODE HERE ***"
        # would just return prediction ... but wants untrained default to 1?
        prediction = np.sign(np.dot(x, self.weights))
        return prediction if prediction else 1
    ...
    def train(self):
        ...
        untrained = True
        while untrained:
            # (while loop's `untrained` parameter is manipulated as it is in order to mimic do-while loop)
            untrained = False
            for x, y in self.get_data_and_monitor(self):
                if self.update(x, y): untrained = True
